      ...
      stages: [
        {
          ...
          cells: [
            ...
            { id: 2, "toVictory": true, neighbor: [1, 3, 5] },
...

        {
          ...
          cells: [
            ...
            { id: 21, "toVictory": true, neighbor: [20, 24] },
        ...
        
        {
          id: 4,
          cells: [
            { id: 28, "toVictory": true, neighbor: [29, 31] },
...

        {
          ...
          cells: [
            ...
            { id: 43, "toVictory": true, neighbor: [40, 44] },
...
export const getLevel = (id: number) => {
  const chosenLevel = defaultDataBase.levels.find(level => level.id === id);
  if (chosenLevel) {
    defaultDataBase.levels.forEach(level => level.isLevelActive = false);
    chosenLevel.isLevelActive = true;
    return chosenLevel;
  } else {
    console.log("Error! such level is not exist")
  }
}
export const finishStage = (id: number) => {
  const activeLevel = defaultDataBase.levels.find(level => level.isLevelActive) as ILevel;
  const doneStage = activeLevel.stages.find(stage => stage.id === id);
  if (doneStage) {
    doneStage.done = true;
  }
}